...
class MainActivity : ComponentActivity() {
    ...
    /**
     * Subir volumen quita el silencio, no solo el número.
     *
     * El mando de una tele no tiene un botón de «sonido»: tiene Volumen +/-, y
     * es lo primero que toca cualquiera que llega a un canal mudo. El sistema
     * ajusta el volumen del aparato solo — eso ya funciona sin tocar nada
     * aquí—, pero eso NO desactiva el `muted` del `<video>`: son dos cosas
     * distintas, y subir el volumen del televisor al máximo sobre un vídeo en
     * silencio sigue sin sonar. Sin este puente, la única salida era encontrar
     * el botón de silencio en la barra con el mando, que es justo lo que
     * costaba.
     *
     * Se fabrica el mismo tipo de evento que `alPulsarAtras`, con el nombre
     * `AudioVolumeUp` — el que ya usa Chromium para esta tecla — y SIN
     * `preventDefault`: el volumen real del televisor tiene que seguir
     * subiendo igual, esto solo añade quitar el silencio encima.
     */
    override fun onKeyDown(keyCode: Int, event: KeyEvent?): Boolean {
        if (keyCode == KeyEvent.KEYCODE_VOLUME_UP && paginaViva) {
            webView.evaluateJavascript(
                """
                (function () {
                  var opciones = { key: 'AudioVolumeUp', code: 'AudioVolumeUp', keyCode: 175, which: 175, bubbles: true };
                  window.dispatchEvent(new KeyboardEvent('keydown', opciones));
                })();
                """.trimIndent(),
                null,
