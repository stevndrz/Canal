...
export async function POST(request: Request) {
  ...
  const [item] = await db.insert(channels).values({ householdId: user.householdId, name, number, category: String(body.category || "Nacional"), description: String(body.description || ""), logoText: String(body.logoText || number).slice(0, 3), color: String(body.color || "#0f766e"), streamUrl: String(body.streamUrl || ""), websiteUrl: String(body.websiteUrl || ""), currentProgram: String(body.currentProgram || "Señal en vivo"), nextProgram: String(body.nextProgram || "Próximamente"), progress: 20 }).returning();
